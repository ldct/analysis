...
theorem Real.LIM_add {a b:ℕ → ℚ} (ha: (a:Sequence).IsCauchy) (hb: (b:Sequence).IsCauchy) :
  ...
  have hab : ((a + b):Sequence).IsCauchy := Sequence.IsCauchy.add ha hb
...
theorem Real.ratCast_sub (a b:ℚ) : (a:Real) - (b:Real) = (a-b:ℚ) := by
  rw [ratCast_def, ratCast_def, ratCast_def]
  rw [Real.LIM.sub (Sequence.IsCauchy.const a) (Sequence.IsCauchy.const b)]
  rw [LIM_eq_LIM]
  intro ε hε
  rw [Rat.eventually_close_of_coe_coe]
  use 0
...
theorem BoundedAwayZero.const {q : ℚ} (hq : q ≠ 0) : BoundedAwayZero fun _ ↦ q := by
  use |q|
  simp [hq]
