...
theorem PreRatZero.eq_iff (a : PreRat) : a ≈ PreRatZero ↔ a.numerator = 0 := by
  simp [PreRatZero]
  rw [PreRat.eq]
  simp
...
lemma Rat.add_of_int (a b:ℤ) : (a:Rat) + (b:Rat) = (a+b:ℤ) := by
  rw [coe_Int_eq, coe_Int_eq, coe_Int_eq, add_eq, eq]
  ring
  all_goals positivity

lemma Rat.mul_of_int (a b:ℤ) : (a:Rat) * (b:Rat) = (a*b:ℤ) := by
...
instance Rat.instInv : Inv Rat where
  inv := Quotient.lift (fun ⟨ a, b, h1 ⟩ ↦ b // a) (by
    intro a b h
    have : a ≈ PreRatZero ∨ ¬ (a ≈ PreRatZero) := Classical.em (a ≈ PreRatZero)
    cases' this with h1 h2
    dsimp
    have h2 : (b ≈ PreRatZero) := Setoid.trans (Setoid.symm h) h1

    simp only [PreRatZero] at h1
    rw [PreRat.eq] at h1
    simp at h1

    simp only [PreRatZero] at h2
    rw [PreRat.eq] at h2
    simp at h2

    rw [h1, h2, formalDiv_zero, formalDiv_zero]
    ...
    rw [PreRatZero.eq_iff] at h2
    rw [PreRatZero.eq_iff] at h3
